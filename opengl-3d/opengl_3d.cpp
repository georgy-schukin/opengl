...
void initVBO() {
    ...
    std::vector<glm::vec3> vertices = randVertices(NUM_OF_VERTICES);
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)(0)); // data for attrib 0 (vertices)
...
void initMVP() {
    glm::mat4 model =
            glm::rotate(rot_x, glm::vec3(-1.0f, 0.0f, 0.0f))*
            glm::rotate(rot_y, glm::vec3(0.0f, -1.0f, 0.0f))*
            glm::mat4(1.0f);
...
void mouse(int button, int state, int x, int y) {
    mouse_pos_x = x;
    mouse_pos_y = y;
    mouse_button_pressed = (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN);
}

void mouseMove(int x, int y) {
    if (mouse_button_pressed) {
        rot_y += GLfloat(x - mouse_pos_x)/10.0f;
        rot_x += GLfloat(y - mouse_pos_y)/10.0f;
        mouse_pos_x = x;
        mouse_pos_y = y;
        initMVP();
        glutPostRedisplay();
...
int main(int argc, char *argv[]) {
    ...
    glutCreateWindow("OpenGL shaders 3D");
